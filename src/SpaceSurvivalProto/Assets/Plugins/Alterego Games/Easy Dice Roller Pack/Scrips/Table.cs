...
public class Table : MonoBehaviour {
	...
	public float SpinPower = 30;				//Amount of random spin
	[Range(0,300)]
		...
		if(checkForResult == false){
			...
			for(int i = 0; i < DiceThrown.Count; i++){
				if(DiceThrown[i].ImAwake == false){
					DiceThrown[i].SpinPower = SpinPower;
	...
	private void SpawnLocation(int currentDieNumer, GameObject currentDie){
		currentDie.transform.localPosition = new Vector3(0 + (1 * countX),0 + (1 * countY),0+ (1 * countZ));
